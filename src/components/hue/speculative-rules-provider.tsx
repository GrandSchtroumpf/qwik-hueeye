...
export const SpeculativeRuleContext = createContextId<SpeculativeRuleConfig[]>(
  "SpeculativeRuleContext"
);

export const useSpeculativeRules = () => useContext(SpeculativeRuleContext);

export const HueEyeSpeculativeRulesProvider = component$(() => {
  ...
  for (const type in map) {
    for (const source in map[type as RuleType]) {
      for (const eagerness in map[type as RuleType][source as RuleSource]) {
        ...
        const rule =
          map[type as RuleType][source as RuleSource][
            eagerness as Eagerness | "default"
          ];
        if (!rule) continue;
        if (eagerness === "default") rules[type as RuleType]!.push(rule);
        else
          rules.prerender!.push({ ...rule, eagerness: eagerness as Eagerness });
      }
    }
  }
  return (
